...
function Section({ id, children }: { id: string, children: React.ReactNode }) {
  return (
    <motion.section 
      ...
      className="min-h-screen flex flex-col justify-center py-24 relative z-10"
    >
      {children}
    </motion.section>
  );
    ...
    <div className="w-full max-w-7xl mx-auto px-6 sm:px-12 flex flex-col text-[var(--color-text-muted)] drop-shadow-md outline-white">
      ...
      <Section id="hero">
        <div className="max-w-3xl">
          <h1 className="font-heading text-5xl md:text-7xl font-bold tracking-tighter text-[var(--color-text-primary)] mb-4">
            Raksh<span className="relative inline-block">
              <span className="absolute top-[0.15em] left-[56%] -translate-x-1/2 w-[0.16em] h-[0.16em] bg-red-500 rounded-full drop-shadow-[0_0_12px_rgba(239,68,68,1)] animate-pulse" />
              ı
            </span>tha N.
          </h1>
          <h2 className="font-heading text-xl md:text-2xl text-[var(--color-accent-cyan)] mb-6 outline-black">
      ...
      <Section id="about">
        <div className="max-w-2xl ml-auto">
          ...
          <h2 className="font-heading text-3xl md:text-4xl font-bold text-[var(--color-text-primary)] mb-8">
      ...
      <Section id="skills">
        <div className="max-w-2xl">
          ...
          <h2 className="font-heading text-3xl md:text-4xl font-bold text-[var(--color-text-primary)] mb-10">
      ...
      <Section id="contact">
        <div className="max-w-5xl mx-auto w-full">
          <div className="relative bg-[var(--color-bg-primary)]/80 backdrop-blur-xl border border-[var(--color-accent-cyan)]/40 p-8 md:p-12 rounded-sm shadow-[0_0_40px_rgba(0,229,255,0.15)] overflow-hidden group">
            ...
            <div className="flex flex-col md:flex-row gap-12 items-center justify-between relative z-10">
              ...
              <div className="flex-1 text-left space-y-6">
                ...
                <h2 className="font-heading text-4xl md:text-5xl font-bold text-[var(--color-text-primary)]">
              ...
              <div className="flex flex-col gap-4 w-full md:w-auto min-w-[280px]">
                ...
                <a 
                  ...
                  className="group/btn relative flex items-center justify-center gap-4 px-8 py-5 bg-[var(--color-bg-primary)] border border-[var(--color-accent-cyan)] text-[var(--color-accent-cyan)] font-mono font-bold uppercase tracking-widest hover:bg-[var(--color-accent-cyan)] hover:text-[var(--color-bg-primary)] transition-all duration-300 rounded-sm shadow-[0_0_15px_rgba(0,229,255,0.2)] hover:shadow-[0_0_30px_rgba(0,229,255,0.5)] active:scale-95"
                >
                ...
                <div className="flex gap-4">
                  <a 
                    ...
                    className="flex-1 flex items-center justify-center gap-2 px-6 py-4 border border-[var(--color-hairline)] bg-[var(--color-bg-primary)]/50 text-[var(--color-text-primary)] font-mono text-sm font-bold uppercase tracking-wider hover:border-[var(--color-text-primary)] hover:bg-[var(--color-text-primary)] hover:text-[var(--color-bg-primary)] transition-all rounded-sm shadow-sm"
                  >
                  ...
                  <a 
                    href="#"
                    className="flex-1 flex items-center justify-center gap-2 px-6 py-4 border border-[var(--color-hairline)] bg-[var(--color-bg-primary)]/50 text-[var(--color-text-primary)] font-mono text-sm font-bold uppercase tracking-wider hover:border-[var(--color-accent-cyan)] hover:text-[var(--color-accent-cyan)] transition-all rounded-sm shadow-sm"
                  >
                    <Download size={16} />
                    Resume
                  </a>
...
function Tag({ label }: { label: string }) {
  return (
    <span className="px-3 py-1.5 bg-[var(--color-bg-primary)]/70 backdrop-blur-md border border-[var(--color-hairline)] text-[var(--color-accent-cyan)] rounded-sm font-bold shadow-sm outline-black">
      {label}
    </span>
  );
}
...
function ProjectItem({ title, desc, tags }: { title: string, desc: string, tags: string[] }) {
  return (
    <div className="group block">
      <h3 className="font-heading text-xl font-bold text-[var(--color-text-primary)] mb-3 outline-black group-hover:text-[var(--color-accent-cyan)] transition-all">{title}</h3>
